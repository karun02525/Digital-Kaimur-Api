...
public class AdminServiceImpl implements AdminService {
    ...
    public ResponseEntity<?> isVerify(VerifyModel venderModel) {
    	String vid =venderModel.getVid();
    	int is_verify =venderModel.getIs_verify();
    	String uid = venderModel.getUid();
    	
        log.info("Admin" + venderModel);
    	
        String message="";
        VenderVerifyModel vendeId = mongoTemplate.findOne(new Query(Criteria.where("_id").is(vid.trim()).and("uid").is(uid.trim())), VenderVerifyModel.class);
        if (vendeId == null) {
            return new ResponseEntity<>(new ResponseModel(false, "User id/Vender id invalid! please try again"), HttpStatus.BAD_REQUEST);
        } else {
            Query query1 = new Query();
            query1.addCriteria(Criteria.where("_id").is(vid.trim()));
            ...
            if(is_verify==1){
                ...
                saveVerifyVender(uid,vid,"Pending",message);
            }
            if(is_verify==2){
                message="Your shop has been active successfully";
                saveVerifyVender(uid,vid,"Approved",message); 
            }
            if(is_verify==3){
                message="Your shop has been rejected";
                saveVerifyVender(uid,vid,"Rejected",message);
            }
            return new ResponseEntity<>(new ResponseModel(true, message), HttpStatus.OK);     
     }
  }
    
    private void saveVerifyVender(String uid,String vid,String title,String message) {
